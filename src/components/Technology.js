...
export default function Technology() {
    ...
    const searchTopic = (event) => {
      if (event.key === 'Enter') {
        dispatch(fetchNews({category: "technology", q: topic}));
        setShowTopic(topic);
        setRender(1);
        setTopic('');
      }
    ...
    useEffect(() => {
        if (!hasRun.current) {
          dispatch(fetchNews({category: "technology", q: ""}));
          hasRun.current = true;
        }
      }, []);

  return (
    <div className='tech-page mt-5 mx-4'>
      ...
      <div className='user-control'>
        <div className="search">
          <input 
              ...
              placeholder="Search for a topic/keyword related to Technology"
              type="text"/>
        </div>
        {render ? (
      ...
      <div className='tech-news flex flex-row flex-wrap justify-center'>
