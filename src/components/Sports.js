...
export default function Sports() {
    ...
    const searchTopic = (event) => {
      if (event.key === 'Enter') {
        dispatch(fetchNews({category: "sports", q: topic}));
        setShowTopic(topic);
        setRender(1);
        setTopic('');
      }
    ...
    useEffect(() => {
        if (!hasRun.current) {
          dispatch(fetchNews({category: "sports", q: ""}));
          hasRun.current = true;
        }
      }, []);

  return (
    <div className='sports-page mt-5 mx-4'>
      ...
      <div className='user-control'>
        <div className="search">
          <input 
              ...
              placeholder="Search for a topic/keyword related to Sports"
              type="text"/>
        </div>
        {render ? (
