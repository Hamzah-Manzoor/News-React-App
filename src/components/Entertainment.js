...
export default function Entertainment() {
    ...
    const searchTopic = (event) => {
      if (event.key === 'Enter') {
        dispatch(fetchNews({category: "entertainment", q: topic}));
        setShowTopic(topic);
        setRender(1);
        setTopic('');
      }
    ...
    useEffect(() => {
        if (!hasRun.current) {
          dispatch(fetchNews({category: "entertainment", q: ""}));
          hasRun.current = true;
        }
      }, []);
  ...
  return (
    <div className='entertainment-page mt-5 mx-4'>
      ...
      <div className='user-control'>
        <div className="search">
          <input 
              ...
              placeholder="Search for a topic/keyword related to Entertainment"
              type="text"/>
        </div>
        {render ? (
      ...
      <div className='entertainment-news flex flex-row flex-wrap justify-center'>
